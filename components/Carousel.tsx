...
const EmblaCarousel = ({ fansLikeDetail }: CarouselProps) => {
  ...
  const slides = fansLikeDetail && fansLikeDetail?.filter((item: any) => item.totalMedias > 0)
      ...
      <div className="flex">
        {slides.slice(0, 5).map((item) => (
          <figure
            ...
            onClick={() => router.push(`/medias/${item.media[0]?.mediaId}`)}
          >
            <Image 
            src={item.imageUrl}
            alt="card"
            ...
            <div className="glassmorphism-black relative z-10 flex flex-col rounded-b-xl p-4">
              <h2 className="text-14 font-semibold text-white-1">{item.media[0]?.mediaTitle}</h2>
